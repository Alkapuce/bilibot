...
from .schema import MsgType, StreamingMessage, DecodeResult, ASREngineConfig, TranscribeResult
from .utils import normalize_language_name, validate_language
from .encoder import QwenAudioEncoder
from .llama import LlamaModel, LlamaContext, LlamaBatch, LlamaSampler, get_token_embeddings_gguf

logger = logging.getLogger("bilibot.gguf_core")
...
class QwenASREngine:
    ...
    def __init__(self, config: ASREngineConfig):
        ...
        if self.verbose: logger.info(f"--- [QwenASR] 初始化引擎 (DML: {config.use_dml}, Vulkan: {config.vulkan_enable}) ---")
        ...
        # 2. 加载识别 LLM
        self.model = LlamaModel(llm_gguf)
        self.embedding_table = get_token_embeddings_gguf(llm_gguf)
        self.ctx = LlamaContext(self.model, n_ctx=config.n_ctx, n_batch=4096, embeddings=False)
    ...
    def shutdown(self):
        if self.verbose: logger.info("--- [QwenASR] 引擎已关闭 ---")
    ...
    def _decode(
        ...
        batch = LlamaBatch(max(total_len * 4, 8192), self.model.n_embd, 1)
        ...
        sampler = LlamaSampler(temperature=temperature, seed=seed)
        ...
        if is_last_chunk and not result.is_aborted:
            ...
            if final_p: 
                logger.debug(f"stream final: {final_p}")
    ...
    def _print_stats(self, stats: dict, audio_duration: float, t_total: float):
        ...
        logger.info(f"\n\n📊 性能统计:")
        logger.info(f"  🔹 RTF (实时率) : {rtf:.3f} (越小越快)")
        logger.info(f"  🔹 音频时长    : {audio_duration:.2f} 秒")
        logger.info(f"  🔹 总处理耗时  : {t_total:.2f} 秒")
        logger.info(f"  🔹 编码耗时    : {stats['encode_time']:.3f} 秒")
        logger.info(f"  🔹 LLM 预填充  : {stats['prefill_time']:.3f} 秒 ({stats['prefill_tokens']} tokens, {pre_speed:.1f} tokens/s)")
        logger.info(f"  🔹 LLM 生成    : {stats['decode_time']:.3f} 秒 ({stats['decode_tokens']} tokens, {gen_speed:.1f} tokens/s)")
